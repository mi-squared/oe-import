var data_table = function( options, onShowDetails, onHideDetails, onAfterDraw ) {
    ...
    var fn_work = function()
    {
    	 ...
    	 oTable = $('#'+tableId).dataTable( {
    	  "bProcessing": false, // Hide processing box
        ...
        $('#'+tableId+' tbody').on( 'click', 'a.active-encounter', function( e ) {
            ...
            $.post(setPatientUrl, {pid: pid, encounter: encounter}, function (response) {
                    ...
                    if ((parent.app_view_model.application_data.patient() !== null) && (pid === parent.app_view_model.application_data.patient().pid())) {
                        parent.app_view_model.application_data.patient().pname(response['patientname']);
                        parent.app_view_model.application_data.patient().pubpid(response['pubpid']);
                        parent.app_view_model.application_data.patient().str_dob(response['str_dob']);

                    } else {
                        var new_patient = new parent.patient_data_view_model(response['patientname'], pid, response['pubpid'], response['str_dob']);
                        parent.app_view_model.application_data.patient(new_patient);
                        parent.app_view_model.application_data.therapy_group(null);
                        // navigateTab(webroot_url+"/interface/patient_file/history/encounters.php","enc", function () {
                        //     tabCloseByName('rev');
                        // });

                        /* close therapy group tabs */
                        parent.tabCloseByName('gdg');
                        attendant_type = 'patient';
                        parent.app_view_model.attendant_template_type('patient-data-template');
                    }
                    ...
                    parent.left_nav.loadFrame( 'referral', 'Referral', 'modules/custom_modules/oe-dashboard/views/forms/referral/view.php?id='+encounter);
                },
            'json');
        });
